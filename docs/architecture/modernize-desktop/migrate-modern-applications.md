...
Новая конфигурация позволяет использовать список пар "имя-значение", которые можно сгруппировать в многоуровневую иерархию. Любое сохраненное значение сопоставляется со строкой, при этом существует встроенная поддержка привязки, позволяющая десериализовать параметры в настраиваемый объект POCO.
