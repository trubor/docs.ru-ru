...
При контейнеризация микрослужбы сначала создается контейнер "Image" (образ). Образ является двоичным представлением кода службы, зависимостей и среды выполнения. Хотя вы можете вручную создать образ с помощью `Docker Build` команды из API DOCKER, лучшим подходом является создание его в рамках автоматизированного процесса сборки.
