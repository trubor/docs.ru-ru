...
<span data-ttu-id="e6a53-134">С помощью ДАПР можно.</span><span class="sxs-lookup"><span data-stu-id="e6a53-134">With Dapr, you can.</span></span> <span data-ttu-id="e6a53-135">Рассмотрим, как ДАПР может добавить наблюдаемые возможности в распределенные приложения.</span><span class="sxs-lookup"><span data-stu-id="e6a53-135">Let's look at how Dapr can add observability to our distributed applications.</span></span>
...
### <a name="distributed-tracing"></a><span data-ttu-id="e6a53-168">Распределенная трассировка</span><span class="sxs-lookup"><span data-stu-id="e6a53-168">Distributed tracing</span></span>
...
#### <a name="use-a-jaeger-or-new-relic-monitoring-back-end"></a><span data-ttu-id="e6a53-238">Использование Жаежер или новой серверной части мониторинга Relic</span><span class="sxs-lookup"><span data-stu-id="e6a53-238">Use a Jaeger or New Relic monitoring back end</span></span>
...
#### <a name="service-discovery"></a><span data-ttu-id="e6a53-264">обнаружение служб;</span><span class="sxs-lookup"><span data-stu-id="e6a53-264">Service discovery</span></span>
...
### <a name="health-status"></a><span data-ttu-id="e6a53-362">Состояние работоспособности</span><span class="sxs-lookup"><span data-stu-id="e6a53-362">Health status</span></span>
...
### <a name="dapr-dashboard"></a><span data-ttu-id="e6a53-389">Панель мониторинга ДАПР</span><span class="sxs-lookup"><span data-stu-id="e6a53-389">Dapr dashboard</span></span>
...
### <a name="custom-health-dashboard"></a><span data-ttu-id="e6a53-418">Настраиваемая панель мониторинга работоспособности</span><span class="sxs-lookup"><span data-stu-id="e6a53-418">Custom health dashboard</span></span>
...
### <a name="application-insights"></a><span data-ttu-id="e6a53-434">Application Insights</span><span class="sxs-lookup"><span data-stu-id="e6a53-434">Application Insights</span></span>

<span data-ttu-id="e6a53-435">Ешопондапр Services также отправляют данные телеметрии непосредственно в Azure Application Insights с помощью пакета SDK для Microsoft Application Insights для .NET Core.</span><span class="sxs-lookup"><span data-stu-id="e6a53-435">eShopOnDapr services also send telemetry directly to Azure Application Insights using the Microsoft Application Insights SDK for .NET Core.</span></span> <span data-ttu-id="e6a53-436">Дополнительные сведения см. в статье [Application Insights Azure для ASP.NET Core приложений](/azure/azure-monitor/app/asp-net-core) в документации Майкрософт.</span><span class="sxs-lookup"><span data-stu-id="e6a53-436">For more information, see [Azure Application Insights for ASP.NET Core applications](/azure/azure-monitor/app/asp-net-core) in the Microsoft docs.</span></span>

## <a name="summary"></a><span data-ttu-id="e6a53-437">Итоги</span><span class="sxs-lookup"><span data-stu-id="e6a53-437">Summary</span></span>

<span data-ttu-id="e6a53-438">Хорошее наблюдаемое значение очень важно при запуске распределенной системы в рабочей среде.</span><span class="sxs-lookup"><span data-stu-id="e6a53-438">Good observability is crucial when running a distributed system in production.</span></span>

<span data-ttu-id="e6a53-439">ДАПР предоставляет различные типы телеметрии, включая распределенную трассировку, ведение журнала, метрики и состояние работоспособности.</span><span class="sxs-lookup"><span data-stu-id="e6a53-439">Dapr provides different types of telemetry, including distributed tracing, logging, metrics, and health status.</span></span>

<span data-ttu-id="e6a53-440">ДАПР создает данные телеметрии только для системных служб ДАПР и сидекарс.</span><span class="sxs-lookup"><span data-stu-id="e6a53-440">Dapr only produces telemetry for the Dapr system services and sidecars.</span></span> <span data-ttu-id="e6a53-441">Данные телеметрии из кода приложения не включаются автоматически.</span><span class="sxs-lookup"><span data-stu-id="e6a53-441">Telemetry from your application code isn't automatically included.</span></span> <span data-ttu-id="e6a53-442">Однако вы можете использовать конкретный пакет SDK, например пакет SDK для Опентелеметри для .NET, чтобы создавать данные телеметрии из кода приложения.</span><span class="sxs-lookup"><span data-stu-id="e6a53-442">You can however use a specific SDK like the OpenTelemetry SDK for .NET to emit telemetry from your application code.</span></span>

<span data-ttu-id="e6a53-443">Данные телеметрии ДАПР создаются в формате с открытым стандартом, поэтому его можно принять с помощью большого набора доступных средств мониторинга.</span><span class="sxs-lookup"><span data-stu-id="e6a53-443">Dapr telemetry is produced in an open-standards based format so it can be ingested by a large set of available monitoring tools.</span></span> <span data-ttu-id="e6a53-444">Примеры: Зипкин, Azure Application Insights, ELK Stack, New Relic и Grafana.</span><span class="sxs-lookup"><span data-stu-id="e6a53-444">Some examples are: Zipkin, Azure Application Insights, the ELK Stack, New Relic, and Grafana.</span></span> <span data-ttu-id="e6a53-445">Руководства по мониторингу приложений ДАПР с конкретными серверными элементами мониторинга см. в статье [мониторинг приложения с помощью ДАПР](https://docs.dapr.io/operations/monitoring/) в документации ДАПР.</span><span class="sxs-lookup"><span data-stu-id="e6a53-445">See [Monitor your application with Dapr](https://docs.dapr.io/operations/monitoring/) in the Dapr documentation for tutorials on how to monitor your Dapr applications with specific monitoring back ends.</span></span>

<span data-ttu-id="e6a53-446">Вам потребуются отходы телеметрии, которые принимают данные телеметрии и публикуют их в серверной части мониторинга.</span><span class="sxs-lookup"><span data-stu-id="e6a53-446">You'll need a telemetry scraper that ingests telemetry and publishes it to the monitoring back end.</span></span>

<span data-ttu-id="e6a53-447">ДАПР можно настроить для выдачи структурированного журнала.</span><span class="sxs-lookup"><span data-stu-id="e6a53-447">Dapr can be configured to emit structured logging.</span></span> <span data-ttu-id="e6a53-448">Структурированное ведение журнала предпочтительнее, так как оно может индексироваться средствами серверного мониторинга.</span><span class="sxs-lookup"><span data-stu-id="e6a53-448">Structured logging is favored as it can be indexed by back-end monitoring tools.</span></span> <span data-ttu-id="e6a53-449">Индексированное ведение журнала позволяет пользователям выполнять расширенные запросы при поиске в журнале.</span><span class="sxs-lookup"><span data-stu-id="e6a53-449">Indexed logging enables users to execute rich queries when searching through the logging.</span></span>

<span data-ttu-id="e6a53-450">ДАПР предлагает панель мониторинга, в которой представлены сведения о службах и конфигурации ДАПР.</span><span class="sxs-lookup"><span data-stu-id="e6a53-450">Dapr offers a dashboard that presents information about the Dapr services and configuration.</span></span>

## <a name="references"></a><span data-ttu-id="e6a53-451">Ссылки</span><span class="sxs-lookup"><span data-stu-id="e6a53-451">References</span></span>

- [<span data-ttu-id="e6a53-452">Azure Application Insights</span><span class="sxs-lookup"><span data-stu-id="e6a53-452">Azure Application Insights</span></span>](/azure/azure-monitor/app/app-insights-overview/)
- [<span data-ttu-id="e6a53-453">Открыть телеметрию</span><span class="sxs-lookup"><span data-stu-id="e6a53-453">Open Telemetry</span></span>](https://opentelemetry.io/)
- [<span data-ttu-id="e6a53-454">зипкин</span><span class="sxs-lookup"><span data-stu-id="e6a53-454">Zipkin</span></span>](https://zipkin.io/)
- [<span data-ttu-id="e6a53-455">Контекст трассировки W3C</span><span class="sxs-lookup"><span data-stu-id="e6a53-455">W3C Trace Context</span></span>](https://www.w3.org/TR/trace-context/)
- [<span data-ttu-id="e6a53-456">Jaeger</span><span class="sxs-lookup"><span data-stu-id="e6a53-456">Jaeger</span></span>](https://www.jaegertracing.io/)
- [<span data-ttu-id="e6a53-457">New Relic</span><span class="sxs-lookup"><span data-stu-id="e6a53-457">New Relic</span></span>](https://newrelic.com/)
- [<span data-ttu-id="e6a53-458">Prometheus</span><span class="sxs-lookup"><span data-stu-id="e6a53-458">Prometheus</span></span>](https://prometheus.io/)
- [<span data-ttu-id="e6a53-459">Grafana</span><span class="sxs-lookup"><span data-stu-id="e6a53-459">Grafana</span></span>](https://grafana.com/grafana/)
- [<span data-ttu-id="e6a53-460">Открытие пакета SDK для телеметрии для .NET</span><span class="sxs-lookup"><span data-stu-id="e6a53-460">Open Telemetry SDK for .NET</span></span>](https://opentelemetry.io/docs/net/)
- [<span data-ttu-id="e6a53-461">Fluentd</span><span class="sxs-lookup"><span data-stu-id="e6a53-461">Fluentd</span></span>](https://www.fluentd.org/)
- [<span data-ttu-id="e6a53-462">Стек ELK</span><span class="sxs-lookup"><span data-stu-id="e6a53-462">ELK stack</span></span>](https://www.elastic.co/elastic-stack)
- [<span data-ttu-id="e6a53-463">Порядк</span><span class="sxs-lookup"><span data-stu-id="e6a53-463">Seq</span></span>](https://datalust.co/seq)
- [<span data-ttu-id="e6a53-464">Serilog</span><span class="sxs-lookup"><span data-stu-id="e6a53-464">Serilog</span></span>](https://serilog.net/)

> [!div class="step-by-step"]
> <span data-ttu-id="e6a53-465">[Назад](bindings.md)
> [Вперед](secrets.md)</span><span class="sxs-lookup"><span data-stu-id="e6a53-465">[Previous](bindings.md)
