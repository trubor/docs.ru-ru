...
## <a name="the-init-accessor"></a>Метод доступа init

Код для создания метода доступа `init` аналогичен коду для создания метода доступа `set`, за исключением того, что используется ключевое слово `init` вместо `set`. Различие заключается в том, что метод доступа `init` можно использовать только в конструкторе или с помощью [инициализатора объекта](object-and-collection-initializers.md).
...
## <a name="hidden-property-example"></a>Пример скрытого свойства
...
## <a name="override-property-example"></a>Пример переопределения свойства
